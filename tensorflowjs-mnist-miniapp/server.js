...
async function loadTrainedModel(modelPath) {
  if (modelPath != null) {
    if (!fs.existsSync(modelPath)) {
      console.log(
        "Model not found. Please, train the model using 'node trainer.js --epochs 1 --model_save_path trained_model'"
      );
      exit(0);
    }
    ...
    console.log(
      "Please, provide the path of the trained model using 'node server.js --model_path MODEL_PATH'"
    );
    exit(0);
  }
}
...
function onNewWebsocketConnection(socket) {
  ...
  socket.on("predictionRequest", async function (msg) {
    ...
    let inputTensor = tf.tensor4d(float32buffer, [
      1,
      IMAGE_HEIGHT,
      IMAGE_WIDTH,
      1,
    ...
    let predictions = await model
      .predict(inputTensor, { verbose: true })
      .data();
...
async function startServer(modelPath) {
  ...
  server.listen(SERVER_PORT, () => {
    ...
    console.log(
      "Open http://localhost:1992/ in your browser to start playing!\n"
    );
  });
}
